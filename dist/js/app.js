...
import { initializeDefaults, getAll, putOne, deleteOne, deleteEntityCascade, deleteMapVersionCascade } from './data/db.js';
...
const state = {
  entities: [], relations: [], media: [], settings: {}, clues: [], reveals: [], knowledge: [], mapVersions: [], mapMarkers: [],
  editorEntity: null, conversionSourceId: null, selectedId: null, selectedMapVersionId: null, markerPlacementLocationId: null, mapZoom: 100,
...
function mediaUrl(item){ if(!item?.blob) return ''; if(!state.mediaObjectUrls.has(item.id)) state.mediaObjectUrls.set(item.id,URL.createObjectURL(item.blob)); return state.mediaObjectUrls.get(item.id); }
...
async function refreshState(){
  ...
  const [entities,relations,media,settingRows,clues,reveals,knowledge,mapVersions,mapMarkers]=await Promise.all([
  ...
  Object.assign(state,{entities,relations,media,clues,reveals,knowledge,mapVersions,mapMarkers});
...
const COLLECTIONS={
  ...
  characters:{title:'Characters',description:'Character profiles, arcs, secrets, knowledge, beliefs, family links, and relationships.',types:['character'],defaultType:'character'},
...
function renderDashboard(){
  ...
  const created=[...active].sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt)).slice(0,6);
  const favorites=active.filter(e=>e.favorite).slice(0,6);
  const openQuestions=active.filter(e=>e.type==='question'&&!['Answered','Shelved'].includes(e.status));
  const questions=openQuestions.slice(0,5);
  ...
  const books=active.filter(e=>e.type==='book').sort((a,b)=>(+a.fields?.order||999)-(+b.fields?.order||999));
  const currentBook=entityById(projectSetting().currentBookId)||books[0];
  main.innerHTML=pageHeader(projectSetting().name||'World Bible','Your private control center for lore, story structure, maps, mysteries, and unanswered questions.','<button class="button primary" data-new-entry="lore">+ New entry</button>')+`
    <div class="grid stats">
      <div class="card"><div class="muted small">Active entries</div><div class="stat-value">${active.length}</div></div>
      <div class="card"><div class="muted small">Canon</div><div class="stat-value">${active.filter(e=>e.status==='Canon').length}</div></div>
      <div class="card"><div class="muted small">Open questions</div><div class="stat-value">${openQuestions.length}</div></div>
      <div class="card"><div class="muted small">Mysteries tracked</div><div class="stat-value">${active.filter(e=>e.type==='mystery').length}</div></div>
    </div>
    <div class="grid two section">
      <section class="card"><div class="section-title"><h2>Quick capture</h2><span class="muted small">Idea inbox</span></div><form id="quick-idea-form" class="quick-capture"><input id="quick-idea" placeholder="City built inside the fossilized remains of…" required /><button class="button primary" type="submit">Save idea</button></form></section>
      <section class="card"><div class="section-title"><h2>Current book</h2><a href="#/story" class="muted small">Story →</a></div>${currentBook?entryListItem(currentBook,'story'):emptyState('No book selected','Create a Book entry, then select it in Settings.')}</section>
    </div>
    <div class="grid three section">
      ...
      <section class="card"><div class="section-title"><h2>Favorites</h2></div><div class="list">${favorites.length?favorites.map(e=>entryListItem(e,'entries')).join(''):emptyState('No favorites yet','Star important entries for quick access.')}</div></section>
    </div>
    <section class="card section"><div class="section-title"><h2>Working views</h2></div><div class="actions"><a class="button" href="#/timeline">Master Timeline</a><a class="button" href="#/maps">Maps</a><a class="button" href="#/reveals">Reveal Board</a><a class="button" href="#/knowledge">Knowledge</a><a class="button" href="#/graphs">Graphs</a></div></section>
    <div class="grid three section">
...
function renderTrilogyOverview(){
  ...
  if(!trilogy) return `<section class="card"><div class="section-title"><div><h2>Trilogy Overview</h2><div class="muted small">One high-level record for the whole series.</div></div><button class="button" data-new-entry="trilogy">Create overview</button></div></section>`;
  return `<section class="card trilogy-overview"><div class="section-title"><div><div class="eyebrow">TRILOGY OVERVIEW</div><h2>${esc(trilogy.name)}</h2></div><button class="button" data-open-entry="${esc(trilogy.id)}" data-open-route="story">Open overview</button></div><div class="muted">${esc(trilogy.summary||trilogy.fields?.centralPremise||'No premise written yet.')}</div></section>`;
}
...
function renderMysteryTracking(entity){
  const clues=state.clues.filter(c=>c.mysteryId===entity.id).sort((a,b)=>compareStoryRefs(a.storyEntityId,b.storyEntityId,state.entities)||(Number(a.order)||999)-(Number(b.order)||999));
...
function renderMiniMedia(media){ if(!media.length) return '<div class="muted small">No media attached.</div>'; return `<div class="media-grid">${media.map(item=>`<div class="media-card">${item.blob?`<img src="${mediaUrl(item)}" alt="${esc(item.title||item.name||'Attached image')}" />`:''}<div class="media-card-body"><strong>${esc(item.title||item.name)}</strong><div class="muted small">${(item.tags||[]).map(t=>`#${esc(t)}`).join(' ')}</div></div></div>`).join('')}</div>`; }

function renderTimeline(){
  const events=activeEntities().filter(e=>e.type==='event'); const eras=activeEntities().filter(e=>e.type==='era').sort((a,b)=>a.name.localeCompare(b.name));
  main.innerHTML=pageHeader('Master Timeline','Events use a display date plus optional sortable start/end values and explicit uncertainty. Era filters do not change your written historical wording.','<button class="button primary" data-new-entry="event">+ Historical Event</button>')+`<section class="card"><div class="filter-row"><select id="timeline-era"><option value="">All eras</option>${eras.map(e=>`<option value="${esc(e.id)}">${esc(e.name)}</option>`).join('')}</select><select id="timeline-certainty"><option value="">All certainty levels</option>${['Exact','Approximate','Range','Traditional','Disputed','Unknown'].map(x=>`<option>${x}</option>`).join('')}</select></div><div id="timeline-results"></div></section>`;
  const update=()=>{ const eraId=$('#timeline-era').value, certainty=$('#timeline-certainty').value; const filtered=events.filter(e=>!eraId||e.fields?.eraId===eraId).filter(e=>!certainty||e.fields?.dateUncertainty===certainty).sort(compareTimelineEvents); const groups=new Map(); for(const event of filtered){ const key=event.fields?.eraId||'__unassigned'; (groups.get(key)||groups.set(key,[]).get(key)).push(event); } $('#timeline-results').innerHTML=filtered.length?[...groups.entries()].map(([key,items])=>`<section class="era-band"><div class="era-band-title">${esc(key==='__unassigned'?'Unassigned era':entityById(key)?.name||'Missing era')}</div><div class="timeline">${items.map(e=>`<div class="timeline-item"><button type="button" class="list-item entry-list-button" data-open-entry="${esc(e.id)}" data-open-route="history"><span><span class="list-title">${esc(e.name)}</span><span class="list-meta">${esc(eventRangeLabel(e))}${e.fields?.dateUncertainty?` • ${esc(e.fields.dateUncertainty)}`:''}</span>${e.summary?`<span class="small timeline-summary">${esc(e.summary)}</span>`:''}</span>${badge(e.status)}</button></div>`).join('')}</div></section>`).join(''):emptyState('No events match','Add historical events or change the filters.'); };
  $('#timeline-era').addEventListener('input',update); $('#timeline-certainty').addEventListener('input',update); update();
}
...
function mapThumb(map){ const version=mapVersionForMap(map.id); const media=version?state.media.find(m=>m.id===version.mediaId):null; return media?.blob?`<img src="${mediaUrl(media)}" alt="" />`:'<span class="map-thumb-empty">▧</span>'; }
...
function renderMaps(selectedMapId){
  ...
  const versions=selected?state.mapVersions.filter(v=>v.mapId===selected.id).sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt)):[];
  ...
  const scope=mapScope(selected);
  const childMaps=selected?maps.filter(m=>m.fields?.parentMapId===selected.id):[];
  const rootMaps=maps.filter(m=>!m.fields?.parentMapId||!entityById(m.fields.parentMapId));
  const listOrder=[...rootMaps,...maps.filter(m=>!rootMaps.includes(m))];
  const markerHtml=markers.map(m=>{ const location=entityById(m.locationId); const drill=preferredMapForLocation(m.locationId,selected?.id); return `<button type="button" class="map-marker ${drill?'has-drilldown':''}" style="left:${Number(m.x)}%;top:${Number(m.y)}%" ${drill?`data-open-map="${esc(drill.id)}"`:`data-open-entry="${esc(m.locationId)}" data-open-route="geography"`} title="${esc(drill?`Open ${drill.name}`:`Open ${location?.name||'location'}`)}"><span></span><b>${esc(location?.name||'Location')}${drill?' ↘':''}</b></button>`; }).join('');

  main.innerHTML=pageHeader('Maps','Build a visual atlas from your own map images. A map can cover the whole world or one specific location, and overview-map pins can drill into detailed regional maps.','<button class="button primary" data-new-entry="map">+ New map record</button>')+`
    <div class="map-layout"><section class="card list-panel atlas-panel"><div class="section-title"><div><h2>Atlas</h2><div class="muted small">World → continent → kingdom → city, or any hierarchy you choose.</div></div></div><div class="atlas-list">${listOrder.length?listOrder.map(m=>atlasMapItem(m,selected?.id)).join(''):emptyState('No maps yet','Create a map record, choose its geographic scope, then upload an image.')}</div>${legacy.length?`<div class="form-section"><h3>Legacy #map media</h3><div class="muted small">${legacy.length} older map image${legacy.length===1?' is':'s are'} still safe in Media. Create a map record and upload/reuse a version when ready.</div></div>`:''}</section>
    <section class="card map-workspace">${selected?`${mapBreadcrumb(selected)}<div class="section-title"><div><div class="eyebrow">${esc(selected.fields?.mapKind||'MAP')} ${scope?`• ${esc(scope.fields?.locationKind||'Location')}`:''}</div><h2>${esc(selected.name)}</h2><div class="muted small">${scope?`Covers ${esc(scope.name)}`:esc(selected.fields?.coverage||'No geographic scope selected yet.')}</div></div><div class="actions"><button class="button" data-edit-entry="${esc(selected.id)}">Edit map</button><button class="button primary" data-add-map-version="${esc(selected.id)}">Upload map image</button></div></div>
      ${versions.length?`<div class="version-strip">${versions.map(v=>`<span class="version-chip-wrap"><button type="button" class="version-chip ${v.id===version?.id?'active':''}" data-map-version="${esc(v.id)}">${esc(v.label||v.variant||'Version')}<span>${esc(v.effectiveDate||fmtDate(v.createdAt))}</span></button><button type="button" class="version-delete" data-delete-map-version="${esc(v.id)}" aria-label="Delete ${esc(v.label||'map version')}">×</button></span>`).join('')}</div>`:''}
      ${media?`<div class="map-toolbar"><label>Place a location pin<select id="marker-location"><option value="">Choose a location…</option>${locations.map(l=>`<option value="${esc(l.id)}">${esc(l.name)}${mapsForLocation(l.id).length?' • has map':''}</option>`).join('')}</select></label><button class="button" id="arm-marker">Place pin</button><div class="map-zoom-controls" aria-label="Map zoom"><button class="button ghost" type="button" data-zoom-map="out">−</button><button class="button ghost" type="button" data-zoom-map="fit">Fit</button><span id="map-zoom-label" class="muted small">${state.mapZoom}%</span><button class="button ghost" type="button" data-zoom-map="in">+</button></div><span class="muted small map-help" id="marker-help">Pins open that location's detailed map when one exists; otherwise they open its lore page.</span></div>
      <div class="map-stage" id="map-stage"><div class="map-canvas" id="map-canvas" data-version-id="${esc(version.id)}" style="width:${Math.max(100,state.mapZoom)}%"><img src="${mediaUrl(media)}" alt="${esc(selected.name)} map" />${markerHtml}</div></div>
      <div class="marker-list section"><div class="section-title"><h3>Locations on this map</h3><span class="muted small">${markers.length}</span></div>${markers.length?markers.map(m=>{const loc=entityById(m.locationId),drill=preferredMapForLocation(m.locationId,selected.id);return `<div class="tracking-row"><div><div>${entityLink(loc,'geography')}</div><div class="muted small">${Number(m.x).toFixed(1)}%, ${Number(m.y).toFixed(1)}%${drill?` • detailed map: ${esc(drill.name)}`:''}</div></div><div class="actions">${drill?`<button class="button ghost" data-open-map="${esc(drill.id)}">Open map</button>`:''}<button class="icon-btn" data-delete-marker="${esc(m.id)}" aria-label="Remove marker">×</button></div></div>`;}).join(''):'<div class="muted small">No locations pinned yet.</div>'}</div>`:emptyState('Upload the visual map','This map record has no image yet. Upload a world map, regional map, city plan, political map, or any other image you want to navigate.',`<button class="button primary" data-add-map-version="${esc(selected.id)}">Upload map image</button>`)}
  ...
  $('#map-canvas')?.addEventListener('click',async event=>{ if(!state.markerPlacementLocationId||event.target.closest('.map-marker')) return; const rect=event.currentTarget.getBoundingClientRect(); const x=((event.clientX-rect.left)/rect.width)*100,y=((event.clientY-rect.top)/rect.height)*100; await putOne('mapMarkers',{id:id(),mapVersionId:event.currentTarget.dataset.versionId,locationId:state.markerPlacementLocationId,x:Math.max(0,Math.min(100,x)),y:Math.max(0,Math.min(100,y)),createdAt:now()}); state.markerPlacementLocationId=null; await refreshState(); renderMaps(selected.id); toast('Map pin added.'); });
}

function renderMedia(){
  main.innerHTML=pageHeader('Media','A reusable local library for maps, sketches, symbols, creatures, architecture, family trees, and other reference images.','<button class="button primary" data-upload-media="">+ Add media</button>')+`<section class="card"><div class="filter-row"><input id="media-search" type="search" placeholder="Search media titles, filenames, or tags…" /></div><div id="media-results"></div></section>`;
  const update=()=>{ const q=($('#media-search')?.value||'').trim().toLowerCase(); const items=state.media.filter(item=>!q||[item.title,item.name,...(item.tags||[])].join(' ').toLowerCase().includes(q)); $('#media-results').innerHTML=items.length?`<div class="media-grid">${items.map(item=>{const mapUse=state.mapVersions.filter(v=>v.mediaId===item.id);return `<article class="media-card">${item.blob?`<img src="${mediaUrl(item)}" alt="${esc(item.title||item.name)}" />`:''}<div class="media-card-body"><strong>${esc(item.title||item.name)}</strong><div class="muted small">${(item.tags||[]).map(t=>`#${esc(t)}`).join(' ')||'No tags'}</div><div class="muted small">Linked to ${(item.entityIds||[]).length} entr${(item.entityIds||[]).length===1?'y':'ies'}${mapUse.length?` • used by ${mapUse.length} map version${mapUse.length===1?'':'s'}`:''}</div>${mapUse.length?'<div class="muted small" style="margin-top:8px">Delete the map version first to remove this image safely.</div>':`<button class="button danger ghost" data-delete-media="${esc(item.id)}" style="margin-top:8px">Remove</button>`}</div></article>`;}).join('')}</div>`:emptyState('No media matches',state.media.length?'Try a different search.':'Images are stored locally and can be linked to multiple entries without duplicate uploads.'); };
...
function renderGraphs(){
  ...
  main.innerHTML=pageHeader('Relationship Graphs','Explore structured links without replacing the underlying entries. Family trees use parent/child relationship records.','')+`<div class="grid two"><section class="card"><div class="section-title"><h2>Relationship graph</h2></div><label>Center entry<select id="graph-root"><option value="">Choose…</option>${entities.map(e=>`<option value="${esc(e.id)}">${esc(e.name)} — ${esc(typeLabel(e.type))}</option>`).join('')}</select></label><div id="relationship-graph" class="section"></div></section><section class="card"><div class="section-title"><h2>Family tree</h2></div><label>Character<select id="family-root"><option value="">Choose…</option>${chars.map(e=>`<option value="${esc(e.id)}">${esc(e.name)}</option>`).join('')}</select></label><div id="family-graph" class="section"></div></section></div>`;
  $('#graph-root').addEventListener('input',e=>$('#relationship-graph').innerHTML=e.target.value?renderRelationshipGraph(e.target.value):''); $('#family-root').addEventListener('input',e=>$('#family-graph').innerHTML=e.target.value?renderFamilyTree(e.target.value):'');
}
...
function renderFamilyTree(rootId){
  const levels=familyLevels(rootId,state.relations,3); if(levels.size<=1) return '<div class="muted small">No parent/child links found. Add parent_of or child_of relationships to build the tree.</div>'; const groups=[...levels.entries()].reduce((acc,[id,level])=>{(acc[level]||=[]).push(id);return acc;},{}); const keys=Object.keys(groups).map(Number).sort((a,b)=>a-b); const width=820,rowH=120,height=Math.max(260,keys.length*rowH+60); const positions={}; keys.forEach((level,row)=>{const ids=groups[level]; ids.forEach((entityId,i)=>{positions[entityId]={x:width*(i+1)/(ids.length+1),y:50+row*rowH};});}); const familyRels=state.relations.filter(r=>['parent_of','child_of'].includes(r.type)&&positions[r.fromId]&&positions[r.toId]); return `<div class="graph-scroll"><svg class="graph-svg" viewBox="0 0 ${width} ${height}">${familyRels.map(r=>`<line x1="${positions[r.fromId].x}" y1="${positions[r.fromId].y}" x2="${positions[r.toId].x}" y2="${positions[r.toId].y}" class="graph-edge"/>`).join('')}${Object.entries(positions).map(([entityId,p])=>{const e=entityById(entityId);return `<g class="graph-click" tabindex="0" role="button" data-open-entry="${esc(entityId)}" data-open-route="characters" aria-label="Open ${esc(e?.name||'character')}"><rect x="${p.x-70}" y="${p.y-24}" width="140" height="48" rx="12" class="${entityId===rootId?'graph-family-root':'graph-family-node'}"/><text x="${p.x}" y="${p.y}" class="graph-label">${esc(e?.name?.slice(0,20)||'Missing')}</text></g>`;}).join('')}</svg></div>`; }
...
function renderSettings(){
  ...
  main.innerHTML=pageHeader('Settings & Data','Project identity, local privacy, backup/restore, and human-readable export.','')+`
    <div class="grid two"><section class="card"><h2>Project</h2><form id="project-settings" class="form-grid" style="margin-top:12px"><label class="full">Project name<input id="setting-project-name" value="${esc(project.name||'')}" /></label><label class="full">Current book<select id="setting-current-book"><option value="">None selected</option>${books.map(b=>`<option value="${esc(b.id)}" ${project.currentBookId===b.id?'selected':''}>${esc(b.name)}</option>`).join('')}</select></label><div class="full"><button class="button primary" type="submit">Save settings</button></div></form></section><section class="card"><h2>Local privacy</h2><p class="prose">Your lore content is stored in this browser. The static app can be hosted behind Cloudflare Access without a remote lore database.</p><p class="muted small">Clearing this site's browser storage, losing the device/profile, or changing origins can remove access to local data. Keep backups.</p></section></div>
...
function openEntryEditor(type='lore',entityId=null,prefill=null){ const existing=entityId?entityById(entityId):null; state.editorEntity=structuredClone(existing||prefill||createEmptyEntity(type)); renderEditorForm(state.editorEntity); entryDialog.showModal(); setTimeout(()=>$('#entry-name')?.focus(),0); }
...
async function saveEntity(event){
  event.preventDefault(); const entity=readEditorEntity(); const errors=validateEntity(entity); if(errors.length) return toast(errors[0]); await putOne('entities',entity); await syncParentRelationship(entity);
  if(state.conversionSourceId){ const source=entityById(state.conversionSourceId); if(source){ await putOne('entities',{...source,status:'Converted',updatedAt:now()}); await putOne('relations',{id:id(),fromId:source.id,toId:entity.id,type:'converted_to',note:'Converted from Idea Inbox',createdAt:now()}); } state.conversionSourceId=null; }
  entryDialog.close(); await refreshState(); toast('Entry saved.'); const r=route(); const collection=COLLECTIONS[r.name]; const destination=collection&&(!collection.types||collection.types.includes(entity.type))?r.name:sectionForType(entity.type); setRoute(destination,entity.id); renderRoute();
}
...
function openRelationEditor(fromId){ $('#relation-from').value=fromId; $('#relation-type').innerHTML=RELATION_TYPES.map(r=>`<option value="${esc(r)}">${esc(r.replaceAll('_',' '))}</option>`).join(''); $('#relation-to').innerHTML=entityOptions(null,'',fromId); $('#relation-note').value=''; relationDialog.showModal(); }
async function saveRelation(event){ event.preventDefault(); const relation={id:id(),fromId:$('#relation-from').value,toId:$('#relation-to').value,type:$('#relation-type').value,note:$('#relation-note').value.trim(),createdAt:now()}; const errors=validateRelation(relation); if(errors.length) return toast(errors[0]); await putOne('relations',relation); relationDialog.close(); await refreshState(); renderRoute(); toast('Link added.'); }
...
async function toggleFavorite(entityId){ const entity=entityById(entityId); if(!entity) return; await putOne('entities',{...entity,favorite:!entity.favorite,updatedAt:now()}); await refreshState(); renderRoute(); }
async function archiveCurrentEntity(){ const entity=readEditorEntity(); if(!entity||!entityById(entity.id)) return; if(!confirm(`Archive “${entity.name}”? It will disappear from normal views but remain recoverable.`)) return; await putOne('entities',{...entity,archivedAt:now(),updatedAt:now()}); entryDialog.close(); await refreshState(); setRoute('archive',entity.id); renderRoute(); toast('Entry archived.'); }
async function unarchiveEntry(entityId){ const entity=entityById(entityId); if(!entity) return; await putOne('entities',{...entity,archivedAt:null,updatedAt:now()}); await refreshState(); setRoute(sectionForType(entity.type),entity.id); renderRoute(); toast('Entry restored.'); }
async function deleteEntityConfirmed(){ const entity=state.editorEntity; if(!entity?.archivedAt) return toast('Archive an entry before permanently deleting it.'); if(!confirm(`Permanently delete archived entry “${entity.name}”? This also removes its structured links and subrecords.`)) return; try{ await deleteEntityCascade(entity.id); entryDialog.close(); await refreshState(); setRoute('archive'); renderRoute(); toast('Entry permanently deleted.'); }catch(error){ toast(error.message||'Could not delete entry.'); } }
...
async function importBackupFile(file){ try{ if(!confirm('Restore this backup and replace the current local database? Export your current data first if you need to keep it.')) return; if(file.name.toLowerCase().endsWith('.zip')||file.type==='application/zip') await restoreZipBackup(file,{replace:true}); else await restoreBackup(JSON.parse(await file.text()),{replace:true}); await refreshState(); renderRoute(); toast('Backup restored.'); }catch(error){ console.error(error); toast(error.message||'Could not restore backup.'); } }

function globalSearch(){ const input=$('#global-search'),popover=$('#search-popover'),q=input.value.trim(); if(!q){popover.classList.add('hidden');return;} const results=searchEntities(activeEntities(),q).slice(0,12); popover.innerHTML=results.length?results.map(e=>`<button type="button" class="search-result" data-global-result="${esc(e.id)}"><span><strong>${esc(e.name)}</strong><span class="muted small">${esc(typeLabel(e.type))} • ${esc(e.summary||'')}</span></span>${badge(e.status)}</button>`).join(''):'<div class="empty-state">No matches.</div>'; popover.classList.remove('hidden'); }
...
function bindStaticEvents(){
  ...
  document.addEventListener('click',async event=>{
    const target=event.target.closest('[data-new-entry],[data-edit-entry],[data-open-entry],[data-toggle-favorite],[data-add-relation],[data-delete-relation],[data-add-media],[data-upload-media],[data-delete-media],[data-global-result],[data-convert-idea],[data-unarchive-entry],[data-add-clue],[data-delete-clue],[data-add-reveal],[data-delete-reveal],[data-add-knowledge],[data-add-knowledge-for],[data-delete-knowledge],[data-add-map-version],[data-map-version],[data-delete-map-version],[data-delete-marker],[data-open-map],[data-zoom-map],[data-new-map-for]');
    if(target){
      ...
      if(target.dataset.deleteMedia){ const used=state.mapVersions.filter(v=>v.mediaId===target.dataset.deleteMedia); if(used.length) toast('This image is used by a map version. Delete that map version first.'); else if(confirm('Remove this media item from the local library?')){ await deleteOne('media',target.dataset.deleteMedia); await refreshState(); renderRoute(); toast('Media removed.'); } }
      ...
      if(target.dataset.openMap){ state.selectedMapVersionId=null; state.mapZoom=100; setRoute('maps',target.dataset.openMap); }
      if(target.dataset.mapVersion){ state.selectedMapVersionId=target.dataset.mapVersion; state.mapZoom=100; renderMaps(route().selected||state.mapVersions.find(v=>v.id===target.dataset.mapVersion)?.mapId); }
      if(target.dataset.deleteMapVersion&&confirm('Delete this map image version and its pins? The underlying map record will remain.')){ const mapId=state.mapVersions.find(v=>v.id===target.dataset.deleteMapVersion)?.mapId||route().selected; await deleteMapVersionCascade(target.dataset.deleteMapVersion); state.selectedMapVersionId=null; await refreshState(); renderMaps(mapId); toast('Map version deleted.'); }
      if(target.dataset.zoomMap){ if(target.dataset.zoomMap==='fit') state.mapZoom=100; else if(target.dataset.zoomMap==='in') state.mapZoom=Math.min(300,state.mapZoom+25); else state.mapZoom=Math.max(100,state.mapZoom-25); const canvas=$('#map-canvas'); if(canvas) canvas.style.width=`${state.mapZoom}%`; if($('#map-zoom-label')) $('#map-zoom-label').textContent=`${state.mapZoom}%`; }
    ...
    if(event.target.id==='export-backup') await exportBackupAction(); if(event.target.id==='export-zip') await exportZipAction(); if(event.target.id==='import-backup') $('#import-file').click(); if(event.target.id==='export-markdown') await exportMarkdownAction(); if(!event.target.closest('.search-wrap')) $('#search-popover').classList.add('hidden');
  });

  document.addEventListener('submit',async event=>{ if(event.target.id==='quick-idea-form') quickIdea(event); if(event.target.id==='project-settings'){ event.preventDefault(); const project={...projectSetting(),name:$('#setting-project-name').value.trim()||'World Bible',currentBookId:$('#setting-current-book').value||null}; await putOne('settings',{key:'project',value:project}); await refreshState(); renderRoute(); toast('Project settings saved.'); } });
}

async function boot(){ await initializeDefaults(); await refreshState(); bindStaticEvents(); if(!location.hash)setRoute('dashboard');else renderRoute(); if('serviceWorker'in navigator&&location.protocol.startsWith('http'))navigator.serviceWorker.register('./sw.js').catch(()=>{}); }
boot().catch(error=>{ console.error(error); main.innerHTML=pageHeader('Could not open workspace','The local database could not be initialized.')+`<section class="card"><div class="prose">${esc(error.message||String(error))}</div></section>`; });
