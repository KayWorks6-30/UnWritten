...
import { initializeDefaults, getAll, putOne, deleteOne, deleteEntityCascade } from './data/db.js';
...
const state = {
  ...
  editorEntity: null, conversionSourceId: null, selectedId: null, selectedMapVersionId: null, markerPlacementLocationId: null,
...
function entryListItem(entity,currentRoute='entries'){
  return `<div class="list-item ${entity.archivedAt?'archived-item':''}" data-open-entry="${esc(entity.id)}" data-open-route="${esc(currentRoute)}"><div><div class="list-title">${esc(entity.name||'Untitled')}</div><div class="list-meta">${esc(typeLabel(entity.type))} • edited ${esc(fmtDate(entity.updatedAt))}${entity.archivedAt?` • archived ${esc(fmtDate(entity.archivedAt))}`:''}</div></div><div class="badges">${entity.favorite?'<span class="badge">★</span>':''}${badge(entity.status)}${entity.archivedAt?'<span class="badge">Archived</span>':''}</div></div>`;
}
...
function renderDashboard(){
  ...
  const questions=active.filter(e=>e.type==='question'&&!['Answered','Shelved'].includes(e.status)).slice(0,5);
...
function renderEntryDetail(entity,routeName='entries'){
  ...
  const relatedHtml=rels.map(rel=>{ const other=entityById(otherEntityId(rel,entity.id)); if(!other) return ''; const dir=relationDirection(rel,entity.id)==='outgoing'?'→':'←'; return `<div class="list-item" data-open-entry="${esc(other.id)}" data-open-route="${esc(other.archivedAt?'archive':sectionForType(other.type))}"><div><div class="list-title">${esc(other.name)}</div><div class="list-meta">${dir} ${esc(rel.type.replaceAll('_',' '))}${rel.note?` • ${esc(rel.note)}`:''}</div></div><button class="icon-btn" data-delete-relation="${esc(rel.id)}" aria-label="Remove link">×</button></div>`; }).join('');
...
function renderMaps(selectedMapId){
  const maps=activeEntities().filter(e=>e.type==='map'); const selected=entityById(selectedMapId)||maps[0]||null; const locations=activeEntities().filter(e=>e.type==='location').sort((a,b)=>a.name.localeCompare(b.name)); const legacy=state.media.filter(m=>(m.tags||[]).some(t=>t.toLowerCase()==='map')&&!state.mapVersions.some(v=>v.mediaId===m.id));
  const version=selected?mapVersionForMap(selected.id):null; const versions=selected?state.mapVersions.filter(v=>v.mapId===selected.id).sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt)):[]; const media=version?state.media.find(m=>m.id===version.mediaId):null; const markers=version?state.mapMarkers.filter(m=>m.mapVersionId===version.id):[];
  main.innerHTML=pageHeader('Maps','Maps are structured records with image-version history. Markers target stable Location IDs, so changing an image does not rewrite geography.','<button class="button primary" data-new-entry="map">+ New map</button>')+`
    <div class="map-layout"><section class="card list-panel"><div class="section-title"><h2>Map records</h2></div><div class="list">${maps.length?maps.map(m=>entryListItem(m,'maps')).join(''):emptyState('No map records yet','Create a map record, then add an image version.')}</div>${legacy.length?`<div class="form-section"><h3>Legacy #map media</h3><div class="muted small">These V0.1 map images remain safe in Media. Create a map record and add a version when you are ready to structure them.</div></div>`:''}</section><section class="card map-workspace">${selected?`<div class="section-title"><div><h2>${esc(selected.name)}</h2><div class="muted small">${esc(selected.fields?.mapKind||'Map')} ${selected.fields?.coverage?`• ${esc(selected.fields.coverage)}`:''}</div></div><div class="actions"><button class="button" data-edit-entry="${esc(selected.id)}">Edit map</button><button class="button primary" data-add-map-version="${esc(selected.id)}">+ Image version</button></div></div>${versions.length?`<div class="version-strip">${versions.map(v=>`<button class="version-chip ${v.id===version?.id?'active':''}" data-map-version="${esc(v.id)}">${esc(v.label||v.variant||'Version')}<span>${esc(fmtDate(v.createdAt))}</span></button>`).join('')}</div>`:''}${media?`<div class="map-toolbar"><label>Marker location<select id="marker-location"><option value="">Choose a location…</option>${locations.map(l=>`<option value="${esc(l.id)}">${esc(l.name)}</option>`).join('')}</select></label><button class="button" id="arm-marker">Place marker</button><span class="muted small" id="marker-help">Select a location, then click Place marker and click the map.</span></div><div class="map-stage" id="map-stage" data-version-id="${esc(version.id)}"><img src="${mediaUrl(media)}" alt="${esc(selected.name)} map" />${markers.map(m=>`<button class="map-marker" style="left:${Number(m.x)}%;top:${Number(m.y)}%" data-open-entry="${esc(m.locationId)}" data-open-route="geography" title="${esc(entityById(m.locationId)?.name||'Location')}"><span></span><b>${esc(entityById(m.locationId)?.name||'Location')}</b></button>`).join('')}</div><div class="marker-list section"><div class="section-title"><h3>Markers</h3><span class="muted small">${markers.length}</span></div>${markers.length?markers.map(m=>`<div class="tracking-row"><div>${entityLink(entityById(m.locationId),'geography')}<div class="muted small">${Number(m.x).toFixed(1)}%, ${Number(m.y).toFixed(1)}%</div></div><button class="icon-btn" data-delete-marker="${esc(m.id)}">×</button></div>`).join(''):'<div class="muted small">No markers on this version.</div>'}</div>`:emptyState('No image version yet','Add the first image version for this map.')}`:emptyState('No maps yet','Create a map record to begin.')}</section></div>`;
  $('#arm-marker')?.addEventListener('click',()=>{ const locationId=$('#marker-location').value; if(!locationId) return toast('Choose a location first.'); state.markerPlacementLocationId=locationId; $('#marker-help').textContent='Marker placement armed — click the map.'; });
  $('#map-stage')?.addEventListener('click',async event=>{ if(!state.markerPlacementLocationId||event.target.closest('.map-marker')) return; const rect=event.currentTarget.getBoundingClientRect(); const x=((event.clientX-rect.left)/rect.width)*100,y=((event.clientY-rect.top)/rect.height)*100; await putOne('mapMarkers',{id:id(),mapVersionId:event.currentTarget.dataset.versionId,locationId:state.markerPlacementLocationId,x:Math.max(0,Math.min(100,x)),y:Math.max(0,Math.min(100,y)),createdAt:now()}); state.markerPlacementLocationId=null; await refreshState(); renderMaps(selected.id); toast('Map marker added.'); });
}
...
function entityOptions(types,value,currentId){ return activeEntities().filter(e=>(!types||types.includes(e.type))&&e.id!==currentId).sort((a,b)=>a.name.localeCompare(b.name)).map(e=>`<option value="${esc(e.id)}" ${String(value)===String(e.id)?'selected':''}>${esc(e.name)} — ${esc(typeLabel(e.type))}</option>`).join(''); }
...
async function saveEntity(event){
  ...
  entryDialog.close(); await refreshState(); toast('Entry saved.'); const r=route(); const destination=COLLECTIONS[r.name]?r.name:sectionForType(entity.type); setRoute(destination,entity.id); renderRoute();
}
...
async function deleteEntityConfirmed(){ const entity=state.editorEntity; if(!entity?.archivedAt) return toast('Archive an entry before permanently deleting it.'); if(!confirm(`Permanently delete archived entry “${entity.name}”? This also removes its structured links and subrecords.`)) return; await deleteEntityCascade(entity.id); entryDialog.close(); await refreshState(); setRoute('archive'); renderRoute(); toast('Entry permanently deleted.'); }
...
function globalSearch(){ const input=$('#global-search'),popover=$('#search-popover'),q=input.value.trim(); if(!q){popover.classList.add('hidden');return;} const results=searchEntities(activeEntities(),q).slice(0,12); popover.innerHTML=results.length?results.map(e=>`<div class="search-result" data-global-result="${esc(e.id)}"><div><strong>${esc(e.name)}</strong><div class="muted small">${esc(typeLabel(e.type))} • ${esc(e.summary||'')}</div></div>${badge(e.status)}</div>`).join(''):'<div class="empty-state">No matches.</div>'; popover.classList.remove('hidden'); }
...
function bindStaticEvents(){
  ...
  document.addEventListener('click',async event=>{
    ...
    if(target){
      ...
      if(target.dataset.deleteMedia&&confirm('Remove this media item from the local library?')){ await deleteOne('media',target.dataset.deleteMedia); await refreshState(); renderRoute(); }
      ...
      if(target.dataset.mapVersion){ state.selectedMapVersionId=target.dataset.mapVersion; renderMaps(route().selected||state.mapVersions.find(v=>v.id===target.dataset.mapVersion)?.mapId); }
