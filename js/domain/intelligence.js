import { compareStoryRefs, storyOrder, storyPath, storyBookId } from './story.js';
import { SEMANTIC_TEXT_REFERENCE_PAIRS } from './schema.js';
import { relationIsProjectable } from './relations.js';

const idOf = value => typeof value === 'string' ? value : value?.id;
const entityMap = entities => new Map((entities||[]).map(e=>[e.id,e]));

export function storyPointId(record){
  return record?.sceneId || record?.chapterId || record?.partId || record?.bookId || record?.storyEntityId || null;
}
...
export function mysteryAnalysis(mysteryId,{clues=[],reveals=[],entities=[]}){
  ...
  const books=new Set(storyRows.map(row=>storyBookId(entities.find(e=>e.id===row.storyId),entities)).filter(Boolean));
  ...
  for(let i=1;i<storyRows.length;i++){
    ...
    const flatten=order=>(order[0]||0)*1e9+(Math.max(0,order[1]||0))*1e6+(Math.max(0,order[2]||0))*1e3+Math.max(0,order[3]||0);
    const flatA=flatten(a),flatB=flatten(b);
...
export function continuityWarnings(data){
  ...
  const projectableRelations=relations.filter(rel=>relationIsProjectable(rel,entities));
  ...
  const parts=active.filter(e=>e.type==='part');
  for(const bookId of new Set(parts.map(part=>part.fields?.parentBookId).filter(Boolean))){
    const seen=new Map();
    for(const part of parts.filter(item=>item.fields?.parentBookId===bookId)){
      const n=String(part.fields?.order||''); if(!n) continue;
      if(seen.has(n)) warnings.push({severity:'warning',code:'part-order',entityId:part.id,message:`${part.name} shares part order ${n} with ${seen.get(n).name}.`}); else seen.set(n,part);
    }
  }
  for(const entity of active){
    for(const [textKey,refKey,label] of SEMANTIC_TEXT_REFERENCE_PAIRS[entity.type]||[]){
      if(String(entity.fields?.[textKey]||'').trim()&&!entity.fields?.[refKey]) warnings.push({severity:'info',code:'legacy-semantic-text',entityId:entity.id,message:`${entity.name} still stores ${label.toLowerCase()} only as legacy text. Convert it to the structured ${label} field so impact tracking and validation remain complete.`});
    }
  }

  const chapters=active.filter(e=>e.type==='chapter');
  for(const chapter of chapters){
    const partId=chapter.fields?.parentPartId; if(!partId) continue;
    const part=byId.get(partId);
    if(part?.type==='part'&&chapter.fields?.parentBookId&&part.fields?.parentBookId!==chapter.fields.parentBookId) warnings.push({severity:'error',code:'chapter-part-book-mismatch',entityId:chapter.id,message:`${chapter.name} points to a Part that belongs to a different Book.`});
  }
  ...
  for(const rel of projectableRelations.filter(r=>r.type==='appears_in')){
  ...
  for(const rel of relations){
    const from=byId.get(rel.fromId),to=byId.get(rel.toId),status=rel.status||'Canon';
    if(status==='Canon'&&['Contradicted','Shelved'].includes(from?.status)) warnings.push({severity:'warning',code:'relationship-endpoint-status',entityId:from.id,message:`A Canon relationship (${rel.type.replaceAll('_',' ')}) still points from ${from.name}, which is ${from.status}.`});
    if(status==='Canon'&&['Contradicted','Shelved'].includes(to?.status)) warnings.push({severity:'warning',code:'relationship-endpoint-status',entityId:to.id,message:`A Canon relationship (${rel.type.replaceAll('_',' ')}) still points to ${to.name}, which is ${to.status}.`});
  }
  ...
  for(const rel of projectableRelations){ const ids=[rel.fromId,rel.toId].sort().join('|'); const arr=relationPairs.get(ids)||[]; arr.push(rel.type); relationPairs.set(ids,arr); }
  ...
  for(const rel of projectableRelations.filter(r=>r.type==='contradicts')){ const a=byId.get(rel.fromId),b=byId.get(rel.toId); if(a?.status==='Canon'&&b?.status==='Canon') warnings.push({severity:'warning',code:'canon-mutual-contradiction',entityId:a.id,message:`${a.name} and ${b.name} are both Canon but are explicitly marked as contradictory.`}); }
  for(const rel of projectableRelations.filter(r=>['precedes','requires'].includes(r.type))){
...
export function sceneContinuity(sceneId,data){
  ...
  const relations=(data.relations||[]).filter(rel=>relationIsProjectable(rel,entities));
  const characters=[];
  for(const rel of relations){
  ...
  const numeric=value=>{const n=Number(value);return value!==''&&value!==null&&value!==undefined&&Number.isFinite(n)?n:null;};
  const date=numeric(scene.fields?.storyDateSort);
  if(date!==null&&location?.type==='location'){
    const from=numeric(location.fields?.existsFrom),to=numeric(location.fields?.existsTo);
    if(from!==null&&date<from) warnings.push({severity:'error',code:'location-before-exists',entityId:scene.id,message:`${scene.name} uses ${location.name} before its recorded existence.`});
    if(to!==null&&date>to) warnings.push({severity:'error',code:'location-after-exists',entityId:scene.id,message:`${scene.name} uses ${location.name} after its recorded end.`});
  }
  if(date!==null) for(const character of uniqueCharacters){
    const birth=numeric(character.fields?.birthSort),death=numeric(character.fields?.deathSort),revival=numeric(character.fields?.revivalSort);
    if(birth!==null&&date<birth) warnings.push({severity:'error',code:'character-before-birth',entityId:scene.id,message:`${character.name} appears in ${scene.name} before their recorded birth.`});
    if(death!==null&&date>death&&(revival===null||date<revival)) warnings.push({severity:'error',code:'character-after-death',entityId:scene.id,message:`${character.name} appears in ${scene.name} after their recorded death and before any recorded return.`});
  }
...
export function plotCoverage({entities=[],workspace=[]},threadId){
  ...
  const scenes=entities.filter(e=>e.type==='scene'&&!e.archivedAt).sort((a,b)=>compareStoryRefs(a.id,b.id,entities));
...
export function characterInteractionMatrix({entities=[],relations=[]}){
  ...
  for(const rel of relations.filter(r=>r.type==='appears_in'&&relationIsProjectable(r,entities))){
...
export function locationUsage(locationId,data){
  ...
  const characters=[];
  for(const rel of relations.filter(r=>relationIsProjectable(r,entities))){ const other=rel.fromId===locationId?rel.toId:rel.toId===locationId?rel.fromId:null; const e=entities.find(x=>x.id===other); if(e?.type==='character') characters.push(e); }
...
export function contentTree(rootId,{entities=[],relations=[]}){
  ...
  for(const rel of relations.filter(r=>['parent_of','contains'].includes(r.type)&&relationIsProjectable(r,entities))){ (children.get(rel.fromId)||children.set(rel.fromId,[]).get(rel.fromId)).push(rel.toId); }
  for(const rel of relations.filter(r=>['child_of','part_of'].includes(r.type)&&relationIsProjectable(r,entities))){ (children.get(rel.toId)||children.set(rel.toId,[]).get(rel.toId)).push(rel.fromId); }
...
export function diplomacyEdges({entities=[],relations=[]},{eraId=null,dateText=''}={}){
  ...
  return relations.filter(r=>politicalTypes.has(r.type)&&relationIsProjectable(r,entities)).filter(r=>!eraId||r.eraId===eraId).filter(r=>!q||[r.activeFrom,r.activeTo,r.note].filter(Boolean).join(' ').toLowerCase().includes(q)).map(r=>({relation:r,from:entities.find(e=>e.id===r.fromId),to:entities.find(e=>e.id===r.toId)})).filter(x=>x.from&&x.to);
}
