...
function renderDashboard(){
  ...
  main.innerHTML=pageHeader(projectSetting().name||'UnWritten.KayWorks','Your private control center for lore, story structure, maps, mysteries, and unanswered questions.','<button class="button primary" data-new-entry="lore">+ New entry</button>')+`
...
async function openEntryEditor(type='lore',entityId=null,prefill=null){ const existing=entityId?entityById(entityId):null; let base=structuredClone(existing||prefill||createEmptyEntity(type)); try{ const draft=await getDraft(base.id); if(draft&&(!existing||draft.baseUpdatedAt===existing.updatedAt)){ base=structuredClone(draft.entity); toast('Recovered unsaved local draft.'); } }catch{} state.editorEntity=base; renderEditorForm(state.editorEntity); entryDialog.showModal(); setTimeout(()=>$('#entry-name')?.focus(),0); }
