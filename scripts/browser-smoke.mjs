...
const server=http.createServer(async(req,res)=>{try{const u=new URL(req.url,'http://local.test');
  if(u.pathname==='/api/health'){res.writeHead(200,{'content-type':'application/json'});return res.end(JSON.stringify({ok:true,ready:true,app:'UnWritten',version:'3.0.0',role:'owner',accessEmail:'owner@local.test'}));}
