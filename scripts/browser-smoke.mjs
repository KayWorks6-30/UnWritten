...
],relations:[],media:[{id:'media-1',name:'world.png',title:'Remote World Map',mime:'image/png',size:68,tags:['map'],entityIds:['map-world'],createdAt:stamp,url:'/api/media/media-1/content'}],settings:[{key:'project',value:{name:'Browser Smoke World',currentBookId:null,schemaVersion:8}}],clues:[],reveals:[],knowledge:[],mapVersions:[{id:'version-1',mapId:'map-world',mediaId:'media-1',label:'Current',variant:'World',effectiveDate:'',notes:'',createdAt:stamp}],mapMarkers:[],workspace:[]};
...
const server=http.createServer(async(req,res)=>{try{const u=new URL(req.url,'http://local.test');
  if(u.pathname==='/api/health'){res.writeHead(200,{'content-type':'application/json'});return res.end(JSON.stringify({ok:true,ready:true,app:'UnWritten',version:'3.5.1',role:'owner',accessEmail:'owner@local.test'}));}
  const storeRecord=u.pathname.match(/^\/api\/store\/([^/]+)\/([^/]+)$/);
  if(storeRecord){const store=decodeURIComponent(storeRecord[1]),key=decodeURIComponent(storeRecord[2]);const records=store==='settings'?snapshot.settings:(snapshot[store]||[]);const record=records.find(row=>(store==='settings'?row.key:row.id)===key);res.writeHead(record?200:404,{'content-type':'application/json'});return res.end(JSON.stringify(record?{ok:true,record}:{ok:false,error:'Record not found.'}));}
  const storeMatch=u.pathname.match(/^\/api\/store\/([^/]+)$/);
  if(storeMatch){const store=decodeURIComponent(storeMatch[1]);const records=store==='settings'?snapshot.settings:(snapshot[store]||[]);res.writeHead(200,{'content-type':'application/json'});return res.end(JSON.stringify({ok:true,records}));}
  if(u.pathname==='/api/entities/query'){const q=(u.searchParams.get('q')||'').toLowerCase();const records=snapshot.entities.filter(e=>!q||`${e.name} ${e.summary}`.toLowerCase().includes(q));res.writeHead(200,{'content-type':'application/json'});return res.end(JSON.stringify({ok:true,records,total:records.length,nextCursor:null}));}
...
const child=spawn(chromium,['--headless=new','--no-sandbox','--disable-gpu','--disable-dev-shm-usage','--disable-background-networking','--no-proxy-server','--no-first-run','--remote-debugging-port=0',`--user-data-dir=${profile}`,'about:blank'],{stdio:['ignore','ignore','pipe']});
...
async function navigate(hash){const loaded=onceEvent('Page.loadEventFired');await call('Page.navigate',{url:`http://127.0.0.1:${port}/${hash}`});await Promise.race([loaded,sleep(5000)]);const result=await call('Runtime.evaluate',{expression:`new Promise(r=>setTimeout(()=>r(document.documentElement.outerHTML),1200))`,awaitPromise:true,returnByValue:true});return result.result?.value||'';}
try{
  await connect();
